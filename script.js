...
function showQuestion() {
    ...
    let html = `<div style="max-width: 500px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background-color: white; border-radius: 12px; padding: 30px 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <p class="question-text" style="font-size: 20px; margin: 0 0 30px 0; line-height: 1.6; color: #333;">${question.question}</p>
...
function showHome() {
  ...
  app.innerHTML = `<div style="max-width: 500px; margin: 0 auto; padding: 20px;">
    ...
    <p style="text-align: center; color: #999; margin-top: 40px; font-size: 14px;">Version 0.1.2</p>
  </div>`;
...
async function showGrammarLesson(lessonId) {
    ...
    if (lesson.sections && lesson.sections.length > 0) {
        lesson.sections.forEach((section, index) => {
            ...
            if (section.content) {
                sectionsHtml += `<div class="lesson-content" style="line-height: 1.8; color: #333; margin-bottom: 15px;">${renderMarkdown(section.content)}</div>`;
            }
            ...
            if (section.examples && section.examples.length > 0) {
                ...
                section.examples.forEach(ex => {
                    sectionsHtml += `<p class="fr-text" style="margin: 8px 0; font-size: 15px;">${renderMarkdown(ex.fr)}</p>`;
                });
                sectionsHtml += `</div>`;
            }
...
function renderTable(table) {
    ...
    let html = `<div style="overflow-x: auto; margin: 15px 0;"><table class="fr-text" style="width: 100%; border-collapse: collapse; font-size: 14px;">`;
...
// توابع کمکی رندر مارک‌داون (اگر قبلاً تعریف نشده‌اند)
...
function showLessonContent(lessonId, section) {
    ...
    let html = `
        <div style="max-width: 500px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); margin-bottom: 20px;">
                <p class="lesson-content" style="line-height: 1.8; color: #333; white-space: pre-line;">${section.content}</p>
    `;
    
    if (section.table) {
        html += `<table class="fr-text" style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 15px;">`;
        html += `<tr>`;
    ...
    if (section.examples && section.examples.length > 0) {
        ...
        section.examples.forEach(example => {
            html += `<div style="background: #f8f9fa; padding: 12px; border-radius: 6px; margin: 8px 0;"><p class="fr-text" style="margin: 0; font-weight: 500;">${example.fr}</p><p style="margin: 4px 0 0 0; font-size: 14px; color: #666;">${renderFaText(example.fa)}</p></div>`;
        });
    }
    ...
    function showCurrentQuestion() {
        ...
        let html = `
            <div style="max-width: 500px; margin: 0 auto; padding: 20px;">
                ...
                <p class="question-text" style="font-size: 18px; line-height: 1.6; color: #333; margin-bottom: 30px;">${renderFaText(question.question)}</p>
